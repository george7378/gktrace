...
Material stringtomat(const string &in, vector <PPMTexture> *const tex)
{
	...
	startpos = endpos + 1; endpos = in.find(",", startpos);
	result.glossAngle = stof(in.substr(startpos, endpos - startpos));

	startpos = endpos + 1; endpos = in.find(";", startpos);
	string surftypeTemp = in.substr(startpos, endpos - startpos);

	if (surftypeTemp == "DIFF"){result.st = DIFF;}
	else if (surftypeTemp == "SPEC"){result.st = SPEC;}
	else if (surftypeTemp == "GLSP"){result.st = GLSP;}
	else if (surftypeTemp == "REFR"){result.st = REFR;}
	else if (surftypeTemp == "GLRF"){result.st = GLRF;}
	else if (surftypeTemp == "LGHT"){result.st = LGHT;}
...
//Parsing
bool parse_scene(const string &fname, RenderProperties *prp, Camera *cam, vector <Shape *> *shp, vector <PPMTexture> *tex)
{
	ifstream scene(fname, ios::in);
	if (!scene.is_open() || !prp || !cam || !shp || !tex){return false;}
	shp->clear(); tex->clear();
	string line = "";
	vector <string> ms, ps, cs, os, ts;
	...
	{
		if (ps[i].find("Width") != string::npos){prp->width = stoi(ps[i].substr(ps[i].find("=") + 1, ps[i].find(";") - ps[i].find("=") - 1));}
		else if (ps[i].find("Height") != string::npos){prp->height = stoi(ps[i].substr(ps[i].find("=") + 1, ps[i].find(";") - ps[i].find("=") - 1));}
		else if (ps[i].find("NumSamples") != string::npos){prp->numsamples = stoi(ps[i].substr(ps[i].find("=") + 1, ps[i].find(";") - ps[i].find("=") - 1));}
		...
		else if (ps[i].find("RussianRoulette") != string::npos){prp->rr = stoi(ps[i].substr(ps[i].find("=") + 1, ps[i].find(";") - ps[i].find("=") - 1)) == 1 ? true : false;}
	}
