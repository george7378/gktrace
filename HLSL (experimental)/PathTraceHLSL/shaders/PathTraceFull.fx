...
float3 randomHemisphereDirection(float3 n)
{
	float r1 = 2*PI*random(float2(n.x, n.z)*RandomModulator), r2 = random(float2(n.y, n.z)*RandomModulator), r2s = sqrt(r2);
	float3 w = n;
...
float3 randomConeDirection(float3 r, float theta)
{
	float r1 = 2*PI*random(float2(r.x, r.z)*RandomModulator), r2 = sin(theta)*sin(theta)*random(float2(r.y, r.z)*RandomModulator), r2s = sqrt(r2);
	float3 w = r;
	float3 u = normalize(cross(abs(w.x) > 0.1f ? float3(0, 1, 0) : float3(1, 0, 0), w));
	float3 v = cross(w, u);
...
//Trace function//
//////////////////
float3 trace(float3 rayorig, float3 raydir)
{
	...
	while (max(max(rayColour.x, rayColour.y), rayColour.z) > scene_Cutoff)
	{
		...
		{
			rayorig = phit + nhit*scene_Bias;
			raydir = randomConeDirection(normalize(reflect(raydir, nhit)), nearestShapeMaterial.glossAngle);
			continue;
		}
		...
		if (nearestShapeMaterial.surfaceType == 5)
		{
			float n = inside ? nearestShapeMaterial.ior : 1/nearestShapeMaterial.ior;
			rayorig = phit - nhit*scene_Bias;
			raydir = randomConeDirection(normalize(refract(raydir, nhit, n)), nearestShapeMaterial.glossAngle);
			continue;
		}
	}
...
PixelColourOut PathTracePixelShader(ScreenQuadVertexToPixel PSIn)
{
	...
	float xx = (2*((PSIn.TexCoords.x*scene_Width + random(PSIn.TexCoords*RandomModulator))/scene_Width) - 1)*fovAngle*scene_Width/scene_Height;
	float yy = (1 - 2*((PSIn.TexCoords.y*scene_Height + random(PSIn.TexCoords*RandomModulator/2))/scene_Height))*fovAngle;
	
	float3 newSample = trace(scene_MainCamera.pos, normalize(scene_MainCamera.left*xx + scene_MainCamera.up*yy + scene_MainCamera.look));
